...
**Snapshot:** 2026-09-25 · `koc` @ this commit (base `5cc80b1`) · 647 leaf
...
**597 of 947 in-scope upstream commands (63%).** Of `koc`'s 647 leaf commands,
597 are upstream-equivalent and 50 are koc-native.
...
**Network flags are checked option by option.**
`internal/cli/network/upstream_flags_test.go` walks the command tree against
every long option python-openstackclient 10.3.0's own parsers define for
`openstack.network.v2` (table generated by `scripts/network-flag-table.py`), and
fails for any an implemented command does not register — so a network command
can no longer count as covered while lacking upstream's filters, which is how
`floating ip list` counted until the network parity pass
(`docs/proposals/network-parity.md`). Post-Zed options are included; on an older
...
| `openstack.network.v2` | 114/168 (68%) | **97/97 (100%)** — QoS and RBAC land outside the "core" denominator but are implemented |
...
| `openstack.network.v2.vpnaas` | 25/25 | 25/25 — VPN services, IKE/IPsec policies, endpoint groups, IPsec site connections |
| `openstack.network.v2.fwaas` | 20/20 | 20/20 — firewall groups, policies, rules |
| `openstack.network.v2.bgpvpn` | 22/22 | 22/22 — BGP VPNs and their network/port/router associations |
...
It returned with neutron tag support in the network parity pass
(`docs/proposals/network-parity.md`), as exactly upstream's `--tag`/`--all-tag`.
...
reconciles with the headline (647 = 597 + 50):
...
```sh
# 1. koc's own command tree (647 leaf commands at the snapshot above)
make build
...
1. every raw row numerator summed = the headline numerator (597);
2. leaf commands = headline numerator + koc-native (647 = 597 + 50);
