...
**Snapshot:** 2026-09-14 · `koc` @ this commit (base `c532e4c`) · 564 leaf
...
**515 of 844 in-scope upstream commands (61%).** Of `koc`'s 564 leaf commands,
515 are upstream-equivalent and 49 are koc-native.
...
No upstream equivalent, by design — **49 leaves**, itemised so the total
reconciles with the headline (564 = 515 + 49):
...
```sh
# 1. koc's own command tree (564 leaf commands at the snapshot above)
make build
...
2. leaf commands = headline numerator + koc-native (564 = 515 + 49);
